...
def get_prompt(query, task, model, instruction=""):
    if task == "MATH" or task == "MATH500":
        ...
        for tem in task_prompts:
            cur.append({"role": "user", "content": tem["Q"]})
            cur.append({"role": "assistant", "content": "Let's think step by step. " + tem["A"]}) # "Let's think step by step. " +
        query = query
    else:
        cur = []
    ...
    if model in ["Mistral-7b-instruct"]:
        cur[0]["content"] = instruction + cur[0]["content"]
        messages = cur + [{"role": "user", "content": query}]
    else:
...
class MyDataset(Dataset):
    def __init__(self, data, model, dataset):
        ...
        if dataset != "MATH" and dataset != "MATH500":
            for item in data:
                prompt = get_prompt(query=item["input"], model=model, task=dataset)
                self.data.append(deepcopy(prompt))
        else:
            for item in data:
                prompt = get_prompt(query=item["problem"], model=model, task=dataset)
                self.data.append(deepcopy(prompt))
...
def all_data(data, model, dataset):
    out_data=[]
    if dataset != "MATH" and dataset != "MATH500":
        for item in data:
            prompt = get_prompt(query=item["input"], model=model, task=dataset)
            out_data.append(deepcopy(prompt))
    else:
        for item in data:
            prompt = get_prompt(query=item["problem"], model=model, task=dataset)
            out_data.append(deepcopy(prompt))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="Mistral-7b-instruct", choices=model_list)
    ...
    if model in model_list:
        print("evaluting {}".format(model))
    else:
        print("no path for {}".format(model))
        sys.exit()


    model_address_map = {
        "Mistral-7b-instruct": "/LM/llama2/FastChat/Mistral-7B-Instruct-v0.3",
    }
    model_id = model_address_map[model]
    ...
    # chat_template = generator.tokenizer.chat_template
    if batch_size > 1:
        generator.tokenizer.pad_token_id = generator.model.config.eos_token_id
    #generator([{"role": "user", "content": "What is the capital of France? Answer in one word."}], do_sample=True, temperature=0, )

    sub_n = 1
    samples = []
    i = 0
    all_temp = [temperature]
    dataset = ["GSM8K", "MATH", "MATH500", "COIN", "SQA", "LETTER", "CSQA"]

    dataset = dataset[0]
    
    if dataset == "GSM8K":
        input_path = "data/GSM8K.json"
        with open(input_path, "r") as f:
            data = json.load(f)
            f.close()

    if dataset == "MATH500":
        input_path = "data/MATH500.jsonl"
        data = []
        with open(input_path, "r") as f:
            for line in f:
                sub_data = json.loads(line)
                data.append(deepcopy(sub_data))
            f.close()
    ...
    if dataset == "COIN":
        input_path = "data/coin_flip.jsonl"
        data = []
        with open(input_path, "r") as f:
            for line in f:
                sub_data = json.loads(line)
    ...
    if dataset == "SQA":
        input_path = "data/strategy.jsonl"
        data = []
        with open(input_path, "r") as f:
            for line in f:
                sub_data = json.loads(line)
    ...
    if dataset == "LETTER":
        input_path = "data/last_letter.jsonl"
        data = []
        with open(input_path, "r") as f:
            for line in f:
                sub_data = json.loads(line)
    ...
    if start_idx > 0:
        data = data[start_idx:]
    # dataset_all = MyDataset(data, model, dataset)
    dataset_all = all_data(data, model, dataset)

    for temp in all_temp:
        samples = []
        print("temperature={}".format(temp))
        if start_idx > 0:
            out_path = "result/{}/{}_samples_n_{}_temp_{}_start_{}.jsonl".format(dataset, model, n, temp, start_idx)
        else:
            out_path = "result/{}/{}_samples_n_{}_temp_{}.jsonl".format(dataset, model, n, temp)

        flag = False
        if dataset != "MATH" and dataset != "MATH500":
            for i in tqdm(range(math.ceil(len(dataset_all)/batch_size))):
                sub_dataset = dataset_all[i*batch_size:(i+1)*batch_size]
                if temperature == 0:
                    completion = generator(sub_dataset, batch_size=batch_size, do_sample=False, temperature=0, max_new_tokens=512, return_full_text=False)
                else:
                    completion = generator(sub_dataset, batch_size=batch_size, do_sample=True, temperature=temperature, num_return_sequences=n, max_new_tokens=512, return_full_text=False)

                for j in range(len(completion)):
                    sub_completion = [completion[j][k]['generated_text'] for k in range(n)]
                    samples.append(dict(input=data[i*batch_size+j]["input"], answer=data[i*batch_size+j]["output"], completion=sub_completion))
                write_jsonl(out_path, samples)
            write_jsonl(out_path, samples)

            # for item in tqdm(data):
            #     prompt = get_prompt(query=item["input"], model=model, task=dataset)
            #     completion = generator(prompt, do_sample=False, temperature=0, max_new_tokens=512)
            #     completion = [completion[0]['generated_text'][-1]['content']]
            #     #print(prompt)
            #     #print(completion)

            #     samples.append(dict(input=item["input"], answer=item["output"], completion=completion))
            ...
            for i in tqdm(range(math.ceil(len(dataset_all)/batch_size))):
                ...
                for j in range(len(completion)):
                    sub_completion = [completion[j][k]['generated_text'] for k in range(n)]
                    samples.append(dict(input=data[i*batch_size+j]["input"], answer=data[i*batch_size+j]["output"], hard_level=data[i*batch_size+j]["level"], subject=data[i*batch_size+j]["subject"], completion=sub_completion))
                write_jsonl(out_path, samples)
            write_jsonl(out_path, samples)
            ...
            #     prompt = get_prompt(query=item["problem"], model=model, task=dataset)
            #     completion = generator(prompt, do_sample=False, temperature=0, max_new_tokens=512)
            #     completion = [completion[0]['generated_text'][-1]]
            #     samples.append(dict(input=item["problem"], answer=item["answer"], hard_level=item["level"],
